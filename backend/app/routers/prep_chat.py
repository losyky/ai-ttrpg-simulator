"""Prep-mode chat endpoint — streams responses from the prep agent via SSE."""
...
from app.agents.prep_agent import run_prep_agent_stream
from app.services import chat_history as ch

router = APIRouter(prefix="/api/prep-chat", tags=["prep-chat"])
...
@router.post("")
async def prep_chat(req: PrepChatRequest):
    async def event_generator():
        try:
            async for chunk in run_prep_agent_stream(
            ...
            log.error("prep_chat error: %s", exc, exc_info=True)
            traceback.print_exc()
            yield {
                "data": ChatResponseChunk(
...
async def get_prep_history(session_id: str):
    return ch.get_history(_AGENT_TYPE, session_id)


@router.patch("/sessions/{session_id}")
async def update_prep_session(session_id: str, body: dict[str, Any]):
    if "label" in body:
...
async def delete_prep_session(session_id: str):
