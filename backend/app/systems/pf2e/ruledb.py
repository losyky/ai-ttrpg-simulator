...
On first connection, if the compendium tables are empty, data is
automatically imported from  systems/pf2e/default_data/rules_seed.json.gz.
"""
...
_RULES_SEED = Path(__file__).resolve().parent / "default_data" / "rules_seed.json.gz"
...
def _seed_rules(conn: sqlite3.Connection) -> None:
    """Import default PF2e rule data from gzipped seed if tables are empty."""
    if not _RULES_SEED.exists():
        return
    cnt = conn.execute("SELECT COUNT(*) FROM compendium_entries").fetchone()[0]
    if cnt > 0:
        return

    print("[PF2e] Seeding rule database from built-in defaults...")
    with gzip.open(str(_RULES_SEED), "rt", encoding="utf-8") as f:
        records = _json_mod.load(f)

    entry_rows = []
    creature_rows = []
    for rec in records:
        tbl = rec.pop("_table", "")
        if tbl == "compendium_entries":
            entry_rows.append(tuple(
                rec.get(c, "") for c in (
                    "id", "key", "name_zh", "name_en", "category",
                    "source_file", "source_label", "description",
                    "description_raw", "prerequisites", "duration",
                    "target", "cost", "extra_json",
                )
            ))
        elif tbl == "creatures":
            creature_rows.append(tuple(
                rec.get(c, "") for c in (
    ...
    if creature_rows:
        ...
        """, creature_rows)
    conn.commit()
    print(f"  Imported {len(entry_rows)} compendium entries + {len(creature_rows)} creatures")
    ...
    try:
        ...
        conn.execute("""
            ...
            SELECT rowid, name_zh, name_en, description, category FROM compendium_entries
        """)
        conn.commit()
        print("  FTS index rebuilt")
    except Exception as exc:
        print(f"  FTS rebuild warning: {exc}")
