...
def search_ancestries(query: str = "") -> list[dict]:
    ...
    if query:
        rows = conn.execute(
            "SELECT * FROM ancestries WHERE name LIKE ? OR slug LIKE ? ORDER BY name",
            (f"%{query}%", f"%{query}%"),
        ).fetchall()
    else:
...
def search_backgrounds(query: str = "", skill: str = "") -> list[dict]:
    ...
    params: list = []
    if query:
        conditions.append("(name LIKE ? OR slug LIKE ?)")
...
def search_classes(query: str = "") -> list[dict]:
    ...
    if query:
        rows = conn.execute(
            "SELECT * FROM classes WHERE name LIKE ? OR slug LIKE ? ORDER BY name",
            (f"%{query}%", f"%{query}%"),
        ).fetchall()
    else:
...
def search_feats(
    ...
    if query:
        conditions.append("(name LIKE ? OR slug LIKE ? OR description LIKE ?)")
